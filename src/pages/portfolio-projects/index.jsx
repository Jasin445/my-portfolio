...
/* ─── Canvas-based particles — zero DOM nodes, one compositor layer ─── */
const Particles = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillStyle = "rgba(107,230,255,0.25)";
...
const PortfolioProjects = () => {
  ...
  // ── Derived data — all in one memo pass ──────────────────────
  ...
  // ── Single memo for all modal navigation state ────────────────
  const { selectedIndex, hasNextProject, hasPrevProject } = useMemo(() => {
    const i = selectedProject
      ? filteredProjects.findIndex((p) => p?.id === selectedProject?.id)
      : -1;
    return {
      selectedIndex: i,
      hasNextProject: i >= 0 && i < filteredProjects.length - 1,
      hasPrevProject: i > 0,
    };
  ...
  // ── CSS-transition filter change — no double-render timer ─────
  ...
  return (
    <>
      ...
      <main className="relative min-h-screen bg-gradient-to-br overflow-hidden from-background via-card to-muted/20">
        ...
        <section
          className="relative z-30 pb-4 sm:py-12 h-full overflow-hidden"
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none">
            <Particles active={animationsActive} />
          </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className="flex-1 min-w-0 py-8">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                  ...
                  <div
                    id="filter-dropdown"
                    className="relative z-30 rounded-lg w-40 sm:w-60"
                  >
                    <button
                    ...
                    >
                      <span>
                        {FILTER_OPTIONS.find((o) => o.value === activeFilter)
                          ?.label ?? "Filter By"}
                      </span>
                      <Icon
